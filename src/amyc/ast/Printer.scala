...
object NominalPrinter extends Printer {
  ...
  override def apply(t: Tree)(implicit printUniqueIDs: Boolean = false): String = {
    ...
    def rec(t: Tree, parens: Boolean = true): Document = t match {
      ...
      case AbstractClassDef(name) =>
        ...
        case CaseClassDef(name, fields, parent) =>
          ...
          "case class " <:> name <:> "(" <:> Lined(fields.map(_.tt) map printField, ", ") <:> ") extends " <:> parent
          ...
          case FunDef(name, params, retType, body) =>
            ...
            case ParamDef(name, tpe, value) =>
              value match {
                case Some(v) => name <:> ": " <:> rec(tpe) <:> "= " <:> rec(v)
                case None => name <:> ": " <:> rec(tpe)
              }
  ...
  override def apply(t: Tree)(implicit printUniqueIDs: Boolean = false): String = {
    ...
    def rec(t: Tree, parens: Boolean = true): Document = t match {
      ...
      case AbstractClassDef(name) =>
        ...
        case CaseClassDef(name, fields, parent) =>
          def printField(f: TypeTree) = "v: " <:> rec(f)
          "case class " <:> printName(name) <:> "(" <:> Lined(fields map printField, ", ") <:> ") extends " <:> printName(parent)

      case FunDef(name, params, retType, body) =>
        Stacked(
          "def " <:> printName(name) <:> "(" <:> Lined(params map (rec(_)), ", ") <:> "): " <:> rec(retType) <:> " = {",
          Indented(rec(body, false)),
          "}"
        )

        case ParamDef(name, tpe) =>
          printName(name) <:> ": " <:> rec(tpe)
      ...
      case Neg(e) =>
        "-(" <:> rec(e) <:> ")"
        case Call(name, args) =>
          printQName(name) <:> "(" <:> Lined(args map (rec(_)), ", ") <:> ")"
      case Sequence(lhs, rhs) =>
        val main = Stacked(
          rec(lhs, false) <:> ";",
          rec(rhs, false),
      ...
      case TypeTree(tp) =>
        tp match {
          ...
          case StringType => "String"
          case UnitType => "Unit"
          case ClassType(name) => printQName(name)
        }
